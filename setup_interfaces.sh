...
while true; do
  ...
  while read interface; do
    ...
    if [[ "$interface_max_rx_ring" != "$interface_current_rx_ring" ]]; then echo "INFO: Setting $interface rx ring to $interface_max_rx_ring"; /usr/sbin/ethtool -G "$interface" rx "$interface_max_rx_ring"; fi
    if [[ "$interface_max_tx_ring" != "$interface_current_tx_ring" ]]; then echo "INFO: Setting $interface tx ring to $interface_max_tx_ring"; /usr/sbin/ethtool -G "$interface" tx "$interface_max_tx_ring"; fi
  ...
  while read interface; do
    ...
    if [[ "$interface_current_mtu" -lt 9000 ]]; then
      echo "INFO: Setting $interface mtu to $interface_max_mtu"
      /bin/ip link set dev "$interface" mtu "$interface_max_mtu"
      if [ $? -ne 0 ]; then
        echo "WARN: Unable to set $interface to mtu $interface_max_mtu. Adjusting down."
        max_mtu=$interface_max_mtu
        mtu_success="1"
        while [[ "$mtu_success" == "1" ]]; do
          max_mtu=$((max_mtu-2))
          echo "INFO: Setting $interface mtu to $max_mtu"
